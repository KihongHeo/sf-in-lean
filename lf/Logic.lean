...
-- Propositions are first-class entities in Lean, though. For
...
-- To prove a conjunction, use the `constructor` tactic (analogous to
-- Rocq's `split`).  This will generate two subgoals, one for each
...
-- true.  The Lean function `And.intro` does this.

#check @And.intro

-- We can use `And.intro` to achieve the same effect as `constructor`.
...
-- `obtain` and just discard the part we don't need.
...
-- analysis -- which can be done with `obtain` using the `|` pattern:
...
-- we can use `exact` or `contradiction` on it to complete any goal:
...
-- find it helpful to `simp only [Not]` or just `intro` near the beginning.
...
theorem zero_not_one : 0 ≠ 1 := by
  ...
  -- `(0 = 1) → False`. To prove it, we assume the opposite...
  intro contra
  -- ... and deduce a contradiction from it.
  -- Here, the equality `0 = 1` contradicts the disjointness of
  -- constructors, so `contradiction` or `nomatch contra` does it.
...
-- achieve an effect similar to that of the `simp`/`omega`/`contradiction`
-- tactics, without literally using them.
...
--
-- The built-in `contradiction` and `omega` tactics take care of this for us.
-- /FULL
...
-- `And` underneath. Lean's `rw` also works with `↔` in `Prop` contexts.
...
-- and the `Iff` type, so no special imports are needed (unlike Rocq's
-- `Setoid` library).
...
theorem mul_eq_0 {n m : Nat} : n * m = 0 ↔ n = 0 ∨ m = 0 := by
  constructor
  · intro h; exact mult_is_O n m h
  · intro h; exact factor_is_O n m h
...
-- existential.  This is done using the `use` tactic (or anonymous
-- constructor syntax `⟨witness, proof⟩`).
...
theorem In_app_iff {A : Type} {l l' : List A} {a : A} :
  ...
  induction l with
  | nil =>
    simp only [List.nil_append, In]
    constructor
    · intro h; right; exact h
    · intro h; obtain h | h := h; exact h.elim; exact h
  | cons x t ih =>
    simp only [List.cons_append, In]
    constructor
    · intro h
      obtain rfl | h := h
      · left; left; rfl
...
#check (@List.reverse : {α : Type _} → List α → List α)

-- We can also use it to check what theorem a particular identifier
-- refers to:

#check @Nat.add_comm  -- ∀ n m, n + m = m + n
...
-- Lean actually allows us to _apply_ a theorem as if it were a
-- function.
--
...
-- FULL
-- Note that we can also write it with explicit instantiation of arguments:
...
-- case where `x` is 42. There are several ways to work around potential
-- issues with inference:

-- We can explicitly provide the witness:
-- in_not_nil_42_take2
...
-- Or we can use it directly as a function:
-- in_not_nil_42_take4
theorem in_not_nil_42_take4 :
  ∀ l : List Nat, In 42 l → l ≠ [] := by
  intro l h
  exact in_not_nil (x := 42) h

-- Or we can explicitly apply the lemma to a hypothesis:
-- in_not_nil_42_take5
...
-- An important difference between Lean and Rocq: In Lean, the
-- `Decidable` type class provides a bridge between `Bool` and `Prop`.
-- When a proposition `P` has a `Decidable P` instance, you can:
--   - Use `P` in `if` expressions
--   - Use the `decide` tactic to prove `P` (or `¬P`) by computation
-- Many propositions on basic types (like `Nat`) are already `Decidable`.
...
theorem even_double_conv : ∀ n, ∃ k,
  ...
  | succ n' ih =>
    ...
    | true =>
      simp
      exact ⟨k, by simp [h] at hk; omega⟩
    | false =>
      simp
      exact ⟨k + 1, by simp [h] at hk; simp [double]; omega⟩
  -- /ADMITTED
-- []
...
theorem eqb_eq {n1 n2 : Nat} :
  Nat.beq n1 n2 = true ↔ n1 = n2 := by
  constructor
  · exact eqb_true n1 n2
  · intro h; subst h; induction n1 with
    | zero => rfl
    | succ n ih => simp only [Nat.beq]; exact ih

-- In Lean, a key advantage is the `Decidable` type class.  For many
-- standard propositions (like `n = m` for natural numbers), Lean
-- automatically provides `Decidable` instances.  This means we can
-- use `decide` to prove or disprove them computationally, and we can
-- use `Prop`-valued expressions in `if` expressions directly!

-- For example, unlike Rocq, we _can_ write:
...
-- even_1000''
example : Even 1000 := by
  exact even_bool_prop.mp rfl

-- In Lean, we can also use `decide` for decidable propositions:
...
--
-- In particular, knowing that `Nat.beq n m = true` is generally of
...
theorem plus_eqb_example {n m p : Nat} :
  Nat.beq n m = true → Nat.beq (n + p) (m + p) = true := by
  -- WORKINCLASS
  intro h
  rw [eqb_eq] at h
  rw [h]
...
theorem eqb_neq {x y : Nat} :
  Nat.beq x y = false ↔ x ≠ y := by
  -- ADMITTED
  constructor
  · intro h hxy
    subst hxy
    have : Nat.beq x x = true := (eqb_eq).mpr rfl
    rw [this] at h
    exact absurd h (by decide)
  · intro h
    cases hbeq : Nat.beq x y with
    | false => rfl
    | true =>
      exfalso
...
-- In Lean (unlike Rocq), functional extensionality is a _theorem_,
-- not an axiom!  The `funext` tactic is built in to Lean's core.
--
...
-- function_equality_ex1
example :
  (fun x => 3 + x) = (fun x => (Nat.pred 4) + x) := by
  rfl

-- These two functions are equal just by simplification, but in general
-- functions can be equal for more interesting reasons.

-- Unlike Rocq, we do NOT need to declare this as an axiom.
-- Lean has `funext` built in!
...
-- We can verify that no axioms are needed:
-- `#print axioms function_equality_ex2` would show only `propext` and `Quot.sound`
-- which are part of Lean's foundations.

-- FULL
-- EX4 (tr_rev_correct)
-- One problem with the definition of the list-reversing function `List.reverse`
...
-- means that `List.reverse` is asymptotically quadratic.
--
...
-- performed; a decent compiler will generate very efficient code in this
-- case.
--
...
theorem rev_append_rev {X : Type} : ∀ (l1 l2 : List X),
  rev_append l1 l2 = l1.reverse ++ l2 := by
  intro l1
  induction l1 with
  | nil => intro l2; rfl
  | cons x l1' ih =>
    intro l2
    simp [rev_append, ih, List.reverse_cons, List.append_assoc]

-- tr_rev_correct
theorem tr_rev_correct {X : Type} : @tr_rev X = @List.reverse X := by
  -- ADMITTED
  funext l
  simp [tr_rev, rev_append_rev, List.append_nil]
  -- /ADMITTED
-- []
-- /FULL
...
-- The following reasoning principle is _not_ derivable in Lean's
-- constructive logic (though it can be added as an axiom, and is
-- available via `Classical.em`):
...
-- However, in Lean, we can access classical reasoning via the
-- `Classical` namespace.  `Classical.em` gives us `∀ P, P ∨ ¬P`,
-- and the `by_contra` tactic allows proof by contradiction.

-- Here's an example using classical reasoning:
...
-- is always safe to assume a decidability axiom for any _particular_ Prop `P`.
...
theorem excluded_middle_irrefutable {P : Prop} :
  ...
  -- ADMITTED
  intro h
  have := de_morgan_not_or h
  obtain ⟨hnp, hnnp⟩ := this
  exact hnnp hnp
  -- /ADMITTED
-- []
...
theorem dne_implies_demorgan :
  double_negation_elimination → de_morgan_not_and_not := by
  intro hdne P Q h
  apply hdne
  intro hnoporq
  exact h ⟨fun hp => hnoporq (Or.inl hp), fun hq => hnoporq (Or.inr hq)⟩

theorem demorgan_implies_em :
...
theorem ito_implies_em :
  ...
  intro hito P
  have := hito P P (fun hp => hp)
  obtain hnp | hp := this
  · right; exact hnp
  · left; exact hp

theorem em_implies_peirce :
